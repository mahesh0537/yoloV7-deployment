...
def put_text_over_image(img, text):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if save_img:
                if dataset.mode == 'image':
                    text = f"Total number of cars = {total_cars}"
                    im0 = put_text_over_image(im0, text)
...
if __name__ == '__main__':
    ...
    json.dump(vars(opt), open('config.json', 'w'))  # save opt to file
    with torch.no_grad():
